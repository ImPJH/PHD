...
def run_trials(mode:str, trials:int, alpha:float, arms:int, lbda:float, horizon:int, latent:np.ndarray, 
               decoder:np.ndarray, reward_params:np.ndarray, noise_dist:Tuple[str], noise_std:List[Union[float, List[float]]], 
    ...
    for trial in range(trials):
        if mode == "full":
            agent = OFUL(d=obs_dim, lbda=lbda, reward_std=cfg.reward_std, delta=cfg.delta)
        else:
            agent = PALO(d=obs_dim, arms=arms, lbda=lbda, reward_std=cfg.reward_std, delta=cfg.delta)
...
def run(mode:str, agent:Union[LinUCB, LineGreedy, PALO], horizon:int, action_size:int, arms:int, latent:np.ndarray, 
...
if __name__ == "__main__":
    ...
    if cfg.seed_mode:
        RESULT_PATH = f"{MOTHER_PATH}/seed_comparison_v3/results/{PATH_DICT[(cfg.mode, path_flag)]}"
        FIGURE_PATH = f"{MOTHER_PATH}/seed_comparison_v3/figures/{PATH_DICT[(cfg.mode, path_flag)]}"
    else:
    ...
    for arms in num_actions:
        K = arms
        log_numerator = T * (cfg.obs_feature_bound ** 2) / cfg.lbda
        log_inside = (1 + log_numerator) / cfg.delta
        if cfg.mode == "full":
            plu_alpha_1 = cfg.reward_std * np.sqrt(d * np.log(log_inside))
            plu_alpha_2 = np.sqrt(cfg.lbda) * cfg.param_bound
            plu_alpha_3 = context_std * np.sqrt((2*d*T*np.log(d*T/cfg.delta)) / cfg.lbda)
            plu_alpha = (plu_alpha_1 + plu_alpha_2 + plu_alpha_3) / np.sqrt(d * np.log(T*K/cfg.delta))
        else:
            plu_alpha = 0
        ALPHAS = [plu_alpha]
        ...
        ## run an experiment
        for alpha in ALPHAS:
            regrets, errors = run_trials(mode=cfg.mode, trials=cfg.trials, alpha=alpha, arms=arms, lbda=cfg.lbda, horizon=T, latent=Z, 
