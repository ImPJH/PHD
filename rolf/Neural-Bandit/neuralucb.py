...
class NeuralUCB(RandomAlg):
    ...
    def __init__(self, d, K, beta=1, lamb=1, hidden_size=32, lr=1e-4, reg=1):
        self.K = K
        self.T = 0
        self.reg = reg
        self.beta = beta
...
class PartialNeuralUCB(RandomAlg):
    def __init__(self, d, K, beta=1, lamb=1, hidden_size=32, lr=1e-4, reg=1):
        self.K = K
        self.T = 0
        self.reg = reg
        self.beta = beta
        self.net = Model(d+K, hidden_size, 1)
        self.hidden_size = hidden_size
        self.net.to(device)
        self.optimizer = optim.Adam(self.net.parameters(), lr=lr)
        self.numel = sum(w.numel() for w in self.net.parameters() if w.requires_grad)
        ...
        self.replay_buffer = ReplayBuffer(d+K, 10000)
