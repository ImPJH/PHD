...
def run_trials(mode:str, trials:int, alpha:float, arms:int, lbda:float, epsilon:float, horizon:int, latent:np.ndarray, 
    ...
    for trial in range(trials):
        if mode == "full":
            ...
            agent = PALO(d=obs_dim, arms=arms, alpha=alpha, lbda=lbda)
...
def run(mode:str, agent:Union[LinUCB, LineGreedy, PALO], horizon:int, action_size:int, arms:int, 
...
if __name__ == "__main__":
    ...
    for arms in num_actions:
        ...
        if cfg.mode == "full":
            ...
            plu_alpha_2 = np.sqrt(cfg.lbda) * cfg.param_bound
            plu_alpha_3 = context_std * np.sqrt((2*(d+K)*T*np.log((d+K)*T/cfg.delta)) / cfg.lbda)
