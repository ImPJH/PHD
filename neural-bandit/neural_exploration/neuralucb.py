...
class NeuralUCB(UCB):
    ...
    def __init__(self,
        ...
        # neural network
        self.model = Model(input_size=bandit.n_obs_features,
...
class NeuralUCB_PO(UCB):
    """Neural UCB with Partial Observability
    """
    def __init__(self,
                 bandit,
                 hidden_size=20,
        ...
        # neural network
        self.model = Model(input_size=bandit.n_obs_features+bandit.n_arms,
                           hidden_size=self.hidden_size,
                           n_layers=self.n_layers,
                           p=self.p
                           ).to(self.device)
        ...
        # bandit.features = np.array([
        #         np.concatenate([bandit.features[t], np.identity(bandit.n_arms)], axis=1) for t in range(bandit.T)
        #     ]).reshape(bandit.T, bandit.n_arms, (bandit.n_obs_features+bandit.n_arms))
        # print(bandit.features.shape)
    ...
    def confidence_multiplier(self):
        """NeuralUCB confidence interval multiplier.
        """
        # # if self.n_optimal_arm >= 1:
        # #     self.confidence_scaling_factor_ = self.confidence_scaling_factor
        # # else:
        # #     self.confidence_scaling_factor_ = self.confidence_scaling_factor * 10
        # self.confidence_scaling_factor_ = self.confidence_scaling_factor * 3 / np.log(self.t+1)
        return (
            self.confidence_scaling_factor
            * np.sqrt(
                self.approximator_dim
