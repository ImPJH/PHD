...
def show_result(regrets:dict, horizon:int, reward_title:str, num_visibles:int, figsize:tuple=(6, 5)):
    ...
    # 그리드 선을 검은색으로 설정하고 표시
    # ax.grid(horizonrue, which='both', axis='both', color='black', linestyle='-', linewidth=0.5)
    ax.grid()
...
if __name__ == "__main__":
    ...
    for m in num_visibles:
        ...
        a = a[:m]
        reward_func = lambda x: cosine(np.dot(a, x))
        ...
        fig = show_result(regrets=regret_dict, horizon=T, reward_title=r"$f_3(\mathbf{x}) = \cos(2\pi\mathbf{x}^\top\mathbf{a})$", num_visibles=m)
        fname = f"seed_{SEED}_cosine_m={m}"
