...
class UCB(abc.ABC):
    ...
    def reset_actions(self):
        ...
        """
        self.actions = np.zeros(self.bandit.T).astype('int')
    ...
    def run(self):
        ...
        with tqdm(total=self.bandit.T, postfix=postfix) as pbar:
            ...
            for t in range(self.bandit.T):
                ...
                self.update_A_inv()
                # compute regret
                self.regrets[t] = self.bandit.best_rewards_oracle[t]-self.bandit.exp_rewards[t, self.action]
                # increment counter
                self.iteration += 1

                # log
                postfix['total regret'] += self.regrets[t]
                self.n_optimal_arm = np.sum(
                    self.actions[:self.iteration] == self.bandit.best_actions_oracle[:self.iteration]
                )
                postfix['% optimal arm'] = '{:.2%}'.format(self.n_optimal_arm / self.iteration)
